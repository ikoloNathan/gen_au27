  ...
  setAttribute(id, attribute) {
    try {
      ...
      if (element !== null) {
        ...
        throw new Error(
          `Element with ID '${id}' not found; attribute not set.`
        );
      }
    } catch (error) {
...
class Body {
  ...
  constructor(select) {
    ...
    this.resetBackoff = this.resetBackoff.bind(this);
    this.nextBackoff = this.nextBackoff.bind(this);

    /** @private */
    this.status_buttons_fn = [
      this.status_button_transmit_fn,
      this.status_button_standby_fn,
      this.status_button_off_fn,
    ];
  }
  ...
  connect() {
    ...
    try {
      ...
      try {
        ...
        return resolve({ ok: false, reason: "constructor-error", error: e });
      }
      websocket.addEventListener("open", () => {
        this.resetBackoff();
        const statusEl = document.getElementById("status");
        if (statusEl) statusEl.textContent = "connected";
        websocket.send(JSON.stringify({ addr: 0, cmd: "ping", t: Date.now() }));
    ...
    this.Document.Document.addEventListener("DOMContentLoaded", () => {
      ...
      fetch("sidebar.json", {
        ...
        .then((jsonArray) => {
          ...
          this.sidebar_table_slot = this.Document.createElement(
            "table",
            null,
            null,
            this.module_ctrl
          ...
          jsonArray.forEach((item, i) => {
            if (item.present === 1) {
              this.sidebar_dividers_outer[i] = this.Document.createElement(
                "div",
                "id_outer_hex_small_" + item.slot,
                "class_outer_hexagon_small Operating",
                this.module_ctrl
              );
              this.sidebar_dividers_inner[i] = this.Document.createElement(
                "div",
                "id_inner_hex_small_" + item.slot,
                "class_inner_hexagon_small enabled",
                this.module_ctrl
              );
              this.sidebar_table_slot_cells[i] =
                this.sidebar_table_slot_row.insertCell(i);
              this.slots[i] = this.Document.createElement(
                "button",
                "id_slot_button_" + item.slot,
                "class_slot_ctrl enabled",
                this.module_ctrl
              );
              ...
              this.slots[i].addEventListener("click", function () {
                ...
                Document.getElementById("main_page").setAttribute(
                  "src",
                  "./module/home_" + item.type + ".htm?slot_id=" + item.slot
                );
              });
            } else {
              ...
              this.sidebar_dividers_outer[i] = this.Document.createElement(
                "div",
                "id_outer_hex_small_" + item.slot,
                "class_outer_hexagon_small disabled",
                this.module_ctrl
              );
              this.sidebar_dividers_inner[i] = this.Document.createElement(
                "div",
                "id_inner_hex_small_" + item.slot,
                "class_inner_hexagon_small disabled",
                this.module_ctrl
              );
              this.sidebar_table_slot_cells[i] =
                this.sidebar_table_slot_row.insertCell(i);
              this.slots[i] = this.Document.createElement(
                "button",
                "id_slot_button_" + type[item.slot],
                "class_slot_ctrl disabled",
                this.module_ctrl
              );
              this.slots[i].innerHTML = item.slot;
            }
            ...
            this.sidebar_dividers_outer[i].appendChild(
              this.sidebar_dividers_inner[i]
            );
            this.sidebar_table_slot_cells[i].appendChild(
              this.sidebar_dividers_outer[i]
            );
          });
        })
    ...
    this.Document.Document.addEventListener("DOMContentLoaded", () => {
      ...
      status_button_li.forEach((button, i) => {
        ...
        if (this.status_buttons[i]) {
          ...
          this.status_buttons[i].addEventListener(
            "click",
            this.status_buttons_fn[i]
          );
        }
      });
      ...
      /** @type {HTMLTableElement} */
      this.sidebar_setting_table = this.Document.createElement(
        "table",
        null,
        null,
        this.settings
      ...
      this.sidebar_setting_cell_upper[0] =
        this.sidebar_settings_rows[0].insertCell(0);
      this.sidebar_setting_cell_upper[1] =
        this.sidebar_settings_rows[0].insertCell(1);
      this.sidebar_setting_cell_lower[0] =
        this.sidebar_settings_rows[1].insertCell(0);
      this.sidebar_setting_cell_lower[1] =
        this.sidebar_settings_rows[1].insertCell(1);
      ...
      for (let i = 0; i < settings_id.length; i++) {
        this.sidebar_settings_div_outer[i] = this.Document.createElement(
          "div",
          "id_outer_hex_medium_" + settings_id[i],
          "class_outer_hexagon_medium Operating",
          this.settings
        );
        this.sidebar_settings_div_inner[i] = this.Document.createElement(
          "div",
          "id_inner_hex_medium_" + settings_id[i],
          "class_inner_hexagon_medium",
          this.settings
        );
        this.sidebar_settings_ctrl[i] = this.Document.createElement(
          "button",
          "id_setting_" + settings_id[i],
          "class_setting_ctrl",
          this.settings
        );
        ...
        this.sidebar_settings_div_inner[i].appendChild(
          this.sidebar_settings_ctrl[i]
        );
        this.sidebar_settings_div_outer[i].appendChild(
          this.sidebar_settings_div_inner[i]
        );
      }
      // Place tiles into the 2x2 table (row-major)
      this.sidebar_setting_cell_upper[0].appendChild(
        this.sidebar_settings_div_outer[0]
      );
      this.sidebar_setting_cell_upper[1].appendChild(
        this.sidebar_settings_div_outer[1]
      );
      this.sidebar_setting_cell_lower[0].appendChild(
        this.sidebar_settings_div_outer[2]
      );
      this.sidebar_setting_cell_lower[1].appendChild(
        this.sidebar_settings_div_outer[3]
      );
    });
  }
  ...
  status_button_transmit_fn() {
    ...
    // this.broker({dest:3,addr:0,cmd:"transmit_state",state:"On"});
    this.sendToServer({ dest: 3, addr: 0, cmd: "transmit_state", state: "On" });
    // websocket.send(
    //   JSON.stringify()
    // );
